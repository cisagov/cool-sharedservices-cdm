...
| cloudwatch\_policy\_instances | Each string corresponds to the name of an instance (as it appears in the CloudWatch log group) for which CDM is to be allowed read access to the specific CloudWatch log groups in which they are interested. | `list(string)` | `[]` | no |
| cloudwatch\_policy\_log\_groups | Each string corresponds to the name of a CloudWatch log group for which CDM is to be allowed read access for selected CLoudWatch log streams. | `list(string)` | `[]` | no |
